...
def stock_dataset_dl(ticker, start, end, ema_short, ema_long, day_data):
    ...
    for i in range(len(df)-5):
        df['label'].iloc[i] = df['Percent_change'].iloc[i+1: i+5].mean() > 0
    ...
    if day_data == True:
        ...
        df_train = df[['price', 'Close', 'high_low', 'Volume', 'ema_st','ema_lg','MACD','rsi','day','daytime','label' ]]
    elif day_data == False: 
        df_train = df[['price', 'Close', 'high_low', 'Volume', 'ema_st','ema_lg','MACD','rsi', 'label']]
    #fill the NaN with 0
