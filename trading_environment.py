...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 15 00:20:28 2021
...
def stock_dataset_dl(ticker, start, end, ema_short, ema_long, day_data= True):
    ...
    df['label'] = df['Percent_change'] = df['Close'].pct_change()
    
    for i in range(len(df)-5):
        if df['Percent_change'].iloc[i+1: i+5].mean() > 0:
            df['label'].iloc[i] = 1
        else:
            df['label'].iloc[i] = 0
    ...
    if day_data == True:
        ...
        df_train = df[['price', 'Close', 'high_low', 'Volume', 'ema_st','ema_lg',
                   'MACD','rsi','day','daytime','Percent_change', 'label' ]]
    else: 
        df_train = df[['price', 'Close', 'high_low', 'Volume', 'ema_st','ema_lg',
                       'MACD','rsi','Percent_change', 'label']]
